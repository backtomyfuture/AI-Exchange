...
async def _enqueue_event_impl(
    queue: asyncio.Queue, ctx, payload: dict, header_event: str | None = None
) -> dict:
    ...
    folder_name = (
        exchange_client.get_folder_name(parent_folder_id) if exchange_client else None
    )
    ...
    if event_type == "NewMailEvent":
        folder_policies = (
            getattr(exchange_client, "_folder_policies", None)
            if exchange_client
            else None
        )
        if folder_policies:
...
class WebhookWorker:
    ...
    async def start(self):
        ...
        logger.info(
            "Exchange webhook worker started (concurrency=%d).", self.concurrency
        )
    ...
    async def enqueue_event(
        self, payload: dict, header_event: str | None = None
    ) -> dict:
