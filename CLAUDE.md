...
## 0. 近期对话沉淀（Lark 收件人编辑 V1，2026-02-13）

本节记录最近一轮高频联调结论，后续开发请优先遵循，避免重复踩坑。

### 0.1 目标与范围（已确认）
- 将卡片编辑态收件人组件由 `select_person` 升级为 `multi_select_person`。
- 编辑态需要带入原有 To/Cc 人员，且可新增人员。
- V1 明确不做全量通讯录分页缓存（组织规模大 + 安全风险）。
- 搜索策略收敛为“邮箱前缀精确搜索”（`@` 前缀），“姓名搜索”已移除。

### 0.2 已落地能力（当前行为）
- 编辑 To/Cc 时可对原有人员进行多选保留/移除。
- 新增人员通过“邮箱前缀搜索”加入候选后勾选。
- 多次搜索结果累计（不会覆盖上一次候选）。
- 外部邮箱/邮件组可手工输入并保存（支持多个）。
- 保存后会回到只读态，草稿收件人写入 `draft_to` / `draft_cc`。
- 原始邮件收件人保持在 `original_to` / `original_cc`，不再被草稿编辑污染。

### 0.3 关键技术决策（必须保持）
- 草稿收件人与原始收件人分离存储：草稿用 `draft_to` / `draft_cc`，原始用 `original_to` / `original_cc`（若无则从 `to` / `cc` 初始化）。
- 人员搜索只走飞书 `contact.v3.user.get`，参数 `user_id_type=user_id`，输入邮箱前缀。
- 搜索结果通过卡片 patch 更新；不 patch 会导致编辑态无法可视化刷新候选。
- 临时编辑态字段（候选、提示、临时已选、外部输入）在保存后清理，避免脏状态残留。

### 0.4 重要踩坑记录
- 报错 `200673` 与卡片结构/提交方式有关，按钮与 `form` 的组合必须符合飞书卡片规范。
- 仅“避免刷新卡片”不可行：会出现按钮状态残留或编辑态不退出。
- 若把草稿收件人误写回原始字段，会出现原始区域显示 `Unknown` 的回归问题。

### 0.5 外部邮箱规则（当前实现）
- 输入支持分隔符：`;`、`；`、`,`、`，`、空格、换行。
- 支持 `Name <email@domain>` 形式，内部会提取邮箱。
- 只读态展示策略：能映射飞书用户则显示 `person` 组件；非飞书地址优先显示 Exchange 解析姓名，解析不到则显示原始邮箱。

### 0.6 关键代码位置（后续改动入口）
- 卡片构建与收件人编辑 UI：`src/utils/card_builder.py`
- 卡片交互回调（搜索/保存/状态持久化）：`src/utils/lark_app.py`
- 实际发信收件人解析（`open_id` -> 邮箱）：`src/nodes/sender.py`

### 0.7 回归测试清单（每次改动后至少覆盖）
- 编辑 To/Cc：仅移除、仅新增、移除+新增混合。
- 连续两次搜索并分别勾选，最终保存后应全部生效。
- 输入外部邮箱（含 `Name <email>`）后保存，重新编辑应可回填。
- 原始收件人区域与草稿收件人区域必须互不影响。
- 历史卡片编辑后状态正确退出（避免“仅取消无保存”或“停留编辑态”）。
