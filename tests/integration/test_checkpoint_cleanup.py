...
RECEIPT_KEY = b"task10-isolated-test-receipt-key" * 2
...
def _cleaner(
    ...
    *,
    ...
    cleaner = CheckpointCleaner(
        ...
        backup_verifier=HmacBackupReceiptVerifier(RECEIPT_KEY),
        now=lambda: clock[0],
    )
...
def _receipt(plan, *, backup_id: str, completed_at: datetime) -> str:
    return create_signed_backup_receipt(
        key=RECEIPT_KEY,
...
async def test_live_style_dry_run_preserves_all_checkpoint_rows_and_hashes(
    ...
    assert (
        tmp_path / "artifacts" / f"{plan.plan_id}.dry-run-report.json"
    ).exists()
...
async def test_same_plan_can_dry_run_then_execute_with_backup_and_quiescence(
    ...
    assert (
        tmp_path / "artifacts" / f"{plan.plan_id}.execute-report.json"
    ).exists()
