...
- 所有数据面事务统一使用：shared account advisory lock -> email -> runtime authority -> 所需 ownership/reservation（generation 升序）-> exact Inbox/card -> Notification -> Mailbox -> Send/effect -> audit/receipt。仅租约路径可以走 account -> ownership -> Inbox，但之后绝不能再锁 email。freeze/switch/control 使用 exclusive account lock 后锁 authority -> ownership；多账户按 account ID 升序。
- Notification、Mailbox、Send 的最后一个远程调用前 guard 都必须先按上述顺序锁定并在同一事务写各自 `request_started_at` 与 `emails.external_effects_started_at=COALESCE(...)`；事务提交后才允许网络调用。邮件级标记本身不递增业务版本，DELETE 只从锁定后的该字段派生“效果已开始”。
...
`notification_outbox` and `mailbox_action_outbox` also contain non-null `generation` and `fencing_token`, plus lease owner/until, attempts, available time, a nullable one-way `request_started_at`, and safe error code. Every Outbox has a unique business key and a foreign key to the owning email/generation. `request_started_at` may be set only by the email-first begin guard and never cleared. `draft_versions` and `send_intents` use a trigger that raises on UPDATE/DELETE outside the dedicated retention role.
...
In one transaction, use untrusted `card_key` only for a non-locking lookup of the candidate trusted resource and its `email_id`; never authorize from that snapshot. Take the shared account advisory lock, then lock email -> current runtime authority -> the exact card ownership row -> exact card resource. Revalidate that the locked card still maps to the locked email before checking the trusted audience/actor and exact signed command/resource binding. This avoids every card/authority -> email inversion against source-delete, draft, notification and send paths. Only an authorized exact-payload retry may then SELECT an existing `approval_actions` row and replay its stored result, even if the resource expired after that action committed; changed payload conflicts. When no action exists, require `source_deleted_at IS NULL`, verify expiry, card/approval version, persisted email/draft identity and hash, email CAS version, and require `durable_active` plus the card's exact authority epoch, generation and fence. Only after all validation succeeds may the transaction insert `approval_actions` by `action_key` and proceed to the business CAS. Environment flags cannot authorize an action.
...
async def test_source_delete_and_send_start_have_one_serialized_winner(
    ...
    if deleted.cancelled_side_effects:
        ...
        assert await repo.unstarted_outbox_statuses(email.id) == {"cancelled"}
        assert await repo.every_later_business_begin_is_rejected(email.id) is True
    assert await repo.started_rows_have_email_marker(email.id) is True


@pytest.mark.integration
async def test_deleted_email_allows_only_card_invalidation_cleanup(
    repo, notification_worker, mailbox_worker, send_worker, lark
):
    email = await repo.seed_deleted_email_with_known_card_and_pending_business_work()
    assert await notification_worker.try_begin_card_delivery(email.card_id) is None
    assert await mailbox_worker.try_begin_attempt(email.mailbox_outbox_id) is None
    assert await send_worker.try_begin_attempt(email.send_outbox_id) is None
    await notification_worker.drain_card_invalidations()
    lark.invalidate.assert_called_once_with(email.message_id)
    assert await repo.card_invalidation_status(email.card_id) in {
        "invalidated",
        "not_found",
    }
...
@pytest.mark.integration
async def test_new_generation_delete_cancels_only_sticky_old_generation_resources(repo):
    email = await repo.seed_old_draining_email_with_unstarted_outboxes(
        owner_generation=4,
        incoming_generation=5,
    )
    deleted = await repo.apply_source_delete_from_current_generation(email.id)
    assert deleted.cancelled_side_effects is True
    assert await repo.outbox_generations(email.id) == {4: {"cancelled"}}
    assert await repo.outbox_count(email.id, generation=5) == 0
```
...
Claim notification rows with generic lease rules and matching generation/fencing token. Immediately before each business Lark delivery/update call, `try_begin_delivery()` takes shared account -> email -> authority/ownership -> exact card/Notification row, revalidates lease/fence and requires `source_deleted_at IS NULL`, sets the Outbox `request_started_at` and the email's one-way external-effect marker in one commit, and returns permission only after commit. A losing delete/cancel path makes zero business Lark calls; once begin wins, DELETE may record source deletion but cannot cancel that delivery. Card cleanup uses a separate operation-aware `try_begin_card_invalidation()` guard with the same lock order: it requires a locally revoked card and a persisted known message ID, permits `source_deleted_at IS NOT NULL`, and can start only the idempotent remote invalidation operation—never a new card delivery/update, approval, mailbox action or send. Call synchronous Lark SDK via `asyncio.to_thread`. Record every known external message ID. Completion verifies lease owner and the same fence. When approval/rejection/expiry/new draft occurs, enqueue updates for all known cards. Duplicate cards remain business-safe because actions share the same command key, DB action authority and CAS.
...
Phase 2's delete decision only carries `cancel_pending_side_effects` and sets `source_deleted_at`. Phase 3 extends the repository transaction using shared account -> email -> current authority -> incoming and sticky ownership rows in generation order -> exact card -> Notification -> Mailbox -> Send. It derives aggregate “started” exclusively from locked `emails.external_effects_started_at`, then validates that every Outbox row with non-null `request_started_at` has that marker; a started-row/null-marker mismatch is a fixed invariant and rolls back. Every Notification/Mailbox/Send begin guard takes the same order and writes both its start fact and the email marker. DELETE preserves every already-started sticky-owner row, cancels every still-unstarted sticky-owner Notification/Mailbox/Send row one by one, and makes every later **business** begin guard reject because `source_deleted_at` is set. The sole post-delete exception is the operation-aware `invalidate_card` cleanup guard described above: it may drain only an already-enqueued local revocation for a persisted known message ID, and it cannot create or revive any business effect. When no row/effect has started, the email may enter `cancelled`; when any effect has started, its business status is preserved, but unrelated unstarted work is still cancelled. Local approval authority is revoked by the deletion/version CAS in both outcomes. Existing known cards receive idempotent invalidation work, and an in-flight notification completion that learns a message ID after deletion must enqueue its invalidation in the same completion transaction; it may never leave a newly delivered deleted-email card actionable. Real PostgreSQL tests cover all three begin-vs-delete races, mixed one-started/multiple-unstarted rows, marker mismatch rollback, approval-vs-delete, post-delete invalidation drain to `invalidated/not_found`, late message-ID invalidation, old-draining/new-current deletion and repeated mixed races with a lock timeout: exactly one legal business remote-effect outcome, stale/revoked actions, terminal cleanup and zero deadlocks.
...
Notification delivered creates `mark_read`; `no_action` creates it after policy state commit. Use generic retry because `mark_read` is idempotent. Claim and complete require the row generation/fencing token and lease owner; a stale Worker cannot commit even if its remote call returns. Immediately before `mark_as_read`, `try_begin_attempt()` takes shared account -> email -> authority/ownership -> exact Mailbox row, rejects deletion/cancellation, and atomically sets the row's one-way `request_started_at` plus `emails.external_effects_started_at`; only its committed success permits the call. Match incoming read projection against `email_id + expected_read_state` to complete pending action without reprocessing the email. Reject unsupported `move` with a permanent capability error.
...
`claim_unstarted()` only returns rows with `request_started_at IS NULL`, matching sticky email generation and current/draining ownership. A new generation cannot claim an old intent. `begin_attempt()` takes shared account -> email -> authority/ownership -> exact Send row, rechecks source deletion, lease owner/generation/fence and commits UUID `attempt_id`, timestamp, `sending`, and `emails.external_effects_started_at=COALESCE(...)` before the network call. Setting the marker does not independently bump the email version; the `sending` authority transition does. If result is unknown, update email/send row/audit and create a high-priority fenced Notification Outbox in one transaction. A stale Worker cannot commit a remote result; an attempt already started is conservatively recorded `send_unknown` by the active recovery coordinator. Recovery changes expired `sending` to `send_unknown`; it never clears either start marker.
...
All three commands require an authorized actor, bounded reason, idempotency key and expected resolution/email version, and reuse `pipeline_command_receipts`. In one transaction they take the shared account lock and then lock in fixed order: email -> current runtime authority -> the attempt's ownership row -> unknown Send attempt/intent. After actor/resource binding, they check the receipt first: same key/same canonical payload returns the stored result without applying current-version checks or doing work, while changed payload conflicts. Only a new command rechecks `send_unknown`, expected version, immutable attempt snapshot, generation/fence and existing resolution. State, one immutable `send_resolution_actions` row, audit and receipt commit together; commit-unknown recovery therefore reads the committed receipt before any retry action.
